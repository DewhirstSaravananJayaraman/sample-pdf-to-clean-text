...
class PDF2Text {
	...
	function decodePDF() {
		...
		if (is_resource($this->filename)) {
			$infile = stream_get_contents($this->filename);
		} else {
			...
			for ($j = 0; $j < strlen($texts[$i]); $j++) {
				...
				switch($c) {
					...
					case "\\":
						...
						$j++;
					break;
					case 'T': // add tabs on text transformations to be able to separate the text later // numeropro
						if (!$isHex and !$isPlain) {
							$document .= "\t";
						}
						// fall through here
					default:
